...
//    $target_file = $target_dir . basename($_FILES["pic4"]["name"]);
    ...
    $sql = mysqli_query($con,"UPDATE `productinfo` SET `category`='$category',`style`='$style',`sku`='$sku',`productName`='$pname',`productDecription`='$pdes',`brand`='$brand',`standardcolor`='$standardColor',`detailedcolor`='$detailedColor',`size`='$size',`LastExportedBy`= '$user',`status`='$status' WHERE `id`='$id'");
